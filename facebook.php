 ...
 *
 * @author iiNTENSE Media <web@iintense.com>
 */
...
class Facebook
{
   ...
   */
  public function getSession($user_session=false, $redirect_url="") {
    if (!$this->sessionLoaded) {
      ...
      if(empty($code)) {
          $access_token = get_option( 'facebookevents-accesstoken' );
          if ( !$access_token ) {

              $pluginurl = $redirect_url;
              $app_login_url = "https://graph.facebook.com/oauth/access_token?client_id=".$this->getAppId()."&client_secret=".$this->getApiSecret()."&grant_type=client_credentials&redirect_uri=" . urlencode($pluginurl);
              //$access_token = $this->makeRequest($token_url,array());
              $access_token = $this->makeRequest($app_login_url,array());
              if ( $access_token ) {
                $data = array();
                parse_str($access_token, $data);
                //echo "Access Token: ";var_dump( $access_token );
                update_option( 'facebookevents-accesstoken', $data['access_token'] );
                return $data;
              }
          ...
          //if ( !$access_token ) {
              ...
              $token_url = "https://graph.facebook.com/oauth/access_token?client_id="
                          . $this->getAppId() . "&redirect_uri=" . urlencode($pluginurl) . "&client_secret="
                          . $this->getApiSecret() . "&code=" . $code;
              $app_login_url = "https://graph.facebook.com/oauth/access_token?
                       client_id=".$this->getAppId()."&client_secret=".$this->getApiSecret()."&
        ...
        if (isset($_COOKIE[$cookieName])) {
          ...
          parse_str(trim(
            /*get_magic_quotes_gpc()
              ...
              : $_COOKIE[$cookieName],*/
              stripslashes($_COOKIE[$cokieName]),
            '"'
          ), $session);
          $session = $this->validateSessionObject($session);
  ...
  public function getAccessToken() {
    ...
    if ($access_token) {
      $expire_duration = (int) get_option( 'facebookevents-accessexpires' );
      $access_time = (int) get_option( 'facebookevents-accesstime' );
      $expire_time = ((int)$expire_duration+(int)$access_time);
      //var_dump($expire_time);
      //var_dump(time());
      $valid = $expire_time-time();
      //var_dump($valid);
      if ($valid > 0) return $access_token;
      update_option( 'facebookevents-accesstoken', '' );
      return "";
    } else {
   ...
   */
  public function getLoginUrl($user_url=false, $redirect_url="", $params=array()) {
    $currentUrl = $this->getCurrentUrl();
    $pluginurl = $redirect_url;
    if ($user_url) {
        $dialog_url = "http://www.facebook.com/dialog/oauth?client_id=" 
                        . $this->getAppId() . "&redirect_uri=" . urlencode($redirect_url)
                        . "&scope=rsvp_event";
        return $dialog_url;
    }
    $dialog_url = "http://www.facebook.com/dialog/oauth?client_id=" 
        . $this->getAppId() . "&redirect_uri=" . urlencode($pluginurl)
        . "&scope=create_event,rsvp_event,offline_access";
    return $dialog_url;
    return $this->getUrl(
      'www',
      'login.php',
